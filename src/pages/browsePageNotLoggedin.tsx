...
const BroswerPage = () => {
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      try {
        setLoading(true);
        const res = await POSTAPI.getPostsWelcome();
        setPosts(res);
  ...
  const content = (
    <div className="browse-page">
      <Navbar/>
      <h1 className="pixelstoreheading" style={{color:"#ddeae0"}}>Welcome to Pixel Store</h1>
      <div className="filter-section">
        ...
        <div className="price-srch">
          <div className="range-label">
            <span className="range-label">Min: {minPrice}</span>
            <span className="range-label">Max: {maxPrice}</span>
          </div>
          <input
            type="range"
            min={0}
      ...
      <div className="post-container">
        {filteredPosts.map((m) => {
          return <Card post={m} key={m.title+Math.random()}/>;
        })}
      </div>
    </div>
  );
